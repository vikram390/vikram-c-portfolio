...
const Navbar: React.FC = () => {
  ...
  const handleLinkClick = (e: React.MouseEvent<HTMLAnchorElement>, href: string) => {
    ...
    // Use a small timeout to allow the menu state change to start processing
    // and prevent mobile browsers from canceling the scroll animation due to layout shifts
    setTimeout(() => {
      if (href === '#' || href === '') {
            ...
            <div className="flex flex-col p-6 gap-6">
              {navLinks.map((link) => (
                <a
                  ...
                  href={link.href}
                  onClick={(e) => handleLinkClick(e, link.href)}
                  className="text-lg font-bold text-gray-700 hover:text-teal-600 transition-colors cursor-pointer"
                >
                  {link.name}
                </a>
              ))}
              <motion.a
                ...
                onClick={(e) => handleLinkClick(e, '#contact')}
                whileTap={{ scale: 0.98 }}
                className="w-full py-4 bg-gray-900 text-white rounded-2xl text-center font-bold active:bg-teal-600 transition-colors cursor-pointer"
              >
                Hire Me
              </motion.a>
            </div>
